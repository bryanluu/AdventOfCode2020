...
# Alternative solver using ndmatrix instead of dictionary
import sys
import numpy as np
...
def initialize_neighbors(seats, positions):
  rows, cols = seats.shape
  neighbors = np.zeros((rows, cols, rows, cols), dtype=bool)
  for r, c in positions[seats != FLOOR]:
    _, neighbors[r, c] = (get_adjacent(seats, r, c) if part == 1 else get_visible_chairs(seats, r, c))
  return neighbors

# Simulate one round of seating
def simulate_round(seats, neighbors, chairs):
  ...
  if np.all(seats != OCCUPIED): # if no seats are occupied
    ...
    occupied[chairs] = np.sum(neighbors[chairs], where=(seats == OCCUPIED), axis=(1, 2))
...
def solve(filename):
  ...
  while True:
    changed, simulation = simulate_round(seats, neighbors, chairs) # simulate round
